...
[TIP]
====
For statuses with a message that are set at all times (for example, the PostgreSQL charm sets an active status with the message "Primary" on the primary unit), consider making an exception and treating these statuses the same as statuses without a message.

In other words, consider giving #4 higher priority than a status with a message that is set at all times
====
